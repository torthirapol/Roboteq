...
#include "roboteq_diff_msgs/msg/duplex.hpp"
#endif
#ifdef _ODOM_COVAR_SERVER
#include "roboteq_diff_msgs/msg/OdometryCovariances.hpp"
#include "rogoteq_diff_msgs/msg/RequestOdometryCovariances.hpp"
#endif
...
{
  // RCLCPP_INFO("Received SIGINT signal, shutting down...");
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Received SIGINT signal, shutting down...");
...
uint32_t millis()
{
	rclcpp::WallTime walltime = rclcpp::WallTime now();
...
public:
  ...
  void odom_publish();
#ifdef _ODOM_COVAR_SERVER
  void odom_covar_callback(const roboteq_diff_msgs::msg::RequestOdometryCovariancesRequest::SharedPtr req, roboteq_diff_msgs::msg::RequestOdometryCovariancesResponse::SharedPtr res);
#endif
...
protected:
  ...
  // tf::msg::TransformBroadcaster odom_broadcaster;
  ...
  rclcpp::TimerBase::SharedPtr odom_covar_server;
  ...
  roboteq_diff_msgs::msg::Duplex current_msg;
  // rclcpp::Publisher current_pub;
  rclcpp::Publisher<roboteq_diff_msgs::msg::Duplex>::SharedPtr current_pub;
  std_msgs::msg::Float32 energy_msg;
  // rclcpp::Publisher energy_pub;
  rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr energy_pub;
  std_msgs::msg::Float32 temperature_msg;
  // rclcpp::Publisher temperature_pub;
  rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr temperature_pub;
#endif
...
void MainNode::cmdvel_callback(const geometry_msgs::msg::Twist::SharedPtr twist_msg) const
{
...
// RCLCPP_DEBUG_STREAM( rclcpp::get_logger(), "Camera Frame size : " << mCamWidth << "x" << mCamHeight);
// RCLCPP_DEBUG_STREAM(rclcpp::get_logger("rclcpp"), "Failed to call service add_two_ints");
#endif
...
#ifdef _CMDVEL_DEBUG
RCLCPP_DEBUG_STREAM(get_logger(),"cmdvel power right: " << right_power << " left: " << left_power);
...
#ifdef _CMDVEL_DEBUG
RCLCPP_DEBUG_STREAM(get_logger(),"cmdvel rpm right: " << right_rpm << " left: " << left_rpm);
#endif
...
void MainNode::cmdvel_setup()
{
  ...
  RCLCPP_INFO_STREAM(get_logger(),"Subscribing to topic " << cmdvel_topic);
  ...
  cmdvel_sub = this->create_subscription<geometry_msgs::msg::Twist>(
      cmdvel_topic, 1000, std::bind(&MainNode::cmdvel_callback, this -1));
}
...
#ifdef _ODOM_COVAR_SERVER
void MainNode::odom_covar_callback(const roboteq_diff_msgs::msg::RequestOdometryCovariancesRequest::SharedPtr req, roboteq_diff_msgs::msg::RequestOdometryCovariancesResponse::SharedPtr res)
{
...
void MainNode::odom_setup()
{
  ...
  if ( pub_odom_tf )
  {
    RCLCPP_INFO(get_logger(),"Broadcasting odom tf");
...
#ifdef _ODOM_COVAR_SERVER
  RCLCPP_INFO("Advertising service on roboteq/odom_covar_srv");
  // odom_covar_server = nh.advertiseService("roboteq/odom_covar_srv", &MainNode::odom_covar_callback, this);
   odom_covar_server = this->create_wall_timer(
      500ms, std::bind(&MainNode::odom_covar_callback, this));
#endif

#ifdef _ODOM_SENSORS
  RCLCPP_INFO(get_logger(),"Publishing to topic roboteq/voltage");
  // voltage_pub = nh.advertise<std_msgs::Float32>("roboteq/voltage", 1000);
  voltage_pub = this->create_publisher<std_msgs::msg::Float32>("roboteq/voltage", 1000);
  RCLCPP_INFO(get_logger(),"Publishing to topic roboteq/current");
  // current_pub = nh.advertise<roboteq_diff_msgs::Duplex>("roboteq/current", 1000);
  current_pub = this->create_publisher<roboteq_diff_msgs::msg::Duplex>("roboteq/current", 1000);
  RCLCPP_INFO(get_logger(),"Publishing to topic roboteq/energy");
  // energy_pub = nh.advertise<std_msgs::Float32>("roboteq/energy", 1000);
  energy_pub = this->create_publisher<std_msgs::msg::Float32>("roboteq/energy", 1000);
  RCLCPP_INFO(get_logger(),"Publishing to topic roboteq/temperature");
  // temperature_pub = nh.advertise<std_msgs::Float32>("roboteq/temperature", 1000);
  temperature_pub = this->create_publisher<std_msgs::msg::Float32>("roboteq/temperature", 1000);
#endif
...
#ifdef _ODOM_DEBUG
RCLCPP_DEBUG_STREAM(get_logger(),"encoder right: " << odom_encoder_right << " left: " << odom_encoder_left);
#endif
            odom_publish();
            break;
          }
...
//  current_msg.header.stamp = rclcpp::Time::now();
...
/*
RCLCPP_DEBUG("right: ");
RCLCPP_DEBUG(odom_encoder_right);
RCLCPP_DEBUG(" left: ");
RCLCPP_DEBUG(odom_encoder_left);
RCLCPP_DEBUG(" dt: ");
RCLCPP_DEBUG(dt);
RCLCPP_DEBUG("");
*/
...
#ifdef _ODOM_DEBUG
//RCLCPP_DEBUG_STREAM( "odom x: " << odom_x << " y: " << odom_y << " yaw: " << odom_yaw );
#endif
...
*/
  ...
  // geometry_msgs::msg::Quaternion quat = tf2::createQuaternionMsgFromYaw(odom_yaw);
  tf2::Quaternion quat;
    quat.setRPY(0, 0, odom_yaw);
  if ( pub_odom_tf )
  {
    rclcpp::Time now = this->get_clock()->now();
    // tf_msg.header.seq++;
    ...
    tf_msg.transform.rotation.w = quat.z();
...
int MainNode::run()
{
	...
	RCLCPP_INFO(get_logger(),"Beginning setup...");
	...
	while ( rclcpp::ok() )
	{
		RCLCPP_INFO_STREAM(get_logger(),"Opening serial port on " << port << " at " << baud << "..." );
		try
		{
			controller.open();
		...
		{
			RCLCPP_WARN_STREAM(get_logger(),"serial::IOException: " << e.what());
		}
		RCLCPP_WARN(get_logger(),"Failed to open serial port");
		sleep(5);
	}
...
//  ros::Rate loop_rate(10);
  ...
  RCLCPP_INFO(get_logger(),"Beginning looping...");
